...
const tagTitlesQuery = `*[_type == "tag"].title`;
 ...
 * @returns {Post} First post matching slug
 */
 ...
 * @param boolean preview render unpublished draft
 * @returns [{_id: ID, slug: string}]
 */
...
export async function allTags(preview = false) {
  const data = await client.fetch(tagTitlesQuery);
  return data;
}
 ...
 * @param {ID} categoryId Category ID
 * @param boolean preview render unpublished draft
 * @returns [{title:string, _id:string}]
 */
